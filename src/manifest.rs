...
//! written atomically. Records live in the WAL, not here (`Shard.records` is
//! `#[serde(skip)]`), so the manifest stays tiny and is safe to rewrite often.
...
/// Atomically write the stream definitions + seq counter to `<dir>/manifest.json`.
